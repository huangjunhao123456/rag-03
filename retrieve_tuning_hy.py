...
def rephrase_retrieve(input:Dict[str,str],llm,retriever, bm25_retriever):
    ...
    # 4.混合检索
    retriever_result = retriever.invoke(rephrase_query,k=5) #相似度检索
    bm25_result = bm25_retriever.invoke(rephrase_query, k=5) #BM25检索

    _result = reciprocal_rank_fusion([retriever_result, bm25_result], k=60, docs_return_num=3)
    return _result
