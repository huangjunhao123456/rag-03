...
def rephrase_retrieve(input:Dict[str,str],llm,retriever, multi_query_num):
    ...
    multi_query_rerank_rrf_chain = (rephrase_chain
                         ...
                         | (lambda x: [retriever.invoke(i) for i in x])  # 遍历检索多个查询
                         | reciprocal_rank_fusion  # RRF重排序
                         )
    retrieve_result = multi_query_rerank_rrf_chain.invoke({"history": input.get("history"),"query": input.get("query")})
